        ...
        private async Task SendReceiptToApiAsync()
        {
            ...
            foreach (var fields in productFieldsList)
            {
                    ...
                    int.TryParse(fields.Quantity.Text, out var quantity))
                {
                    ...
                    sumOfPrices += (amount * quantity);
                }
            }
            if (Convert.ToDecimal(totalAmountField.Text) != sumOfPrices)
            {
                await Application.Current.MainPage.DisplayAlert("Error", "The total price of\nthe products does not equal\nthe total amount of \nthe receipt!", "ОК");
            }
            else
            {
                var requestData = new
